...
export default function MCard({ movie }) {
  const { dispatch, isOnWatchList, isOnStarredList } = useData();
  const { id, title, imageURL, summary } = movie;

  return (
    <div
      key={id}
      className="w-[310px] h-auto flex flex-col justify-between text-left"
    >
      <NavLink to={`/movie/${id}`}>
        <div>
          <img
      ...
      <div className="flex flex-col justify-between">
        ...
        <div className="buttons">
          ...
          <button
            onClick={() => {
              dispatch({ type: "WATCHLIST", payload: id });
            }}
          >
            {isOnWatchList(id) ? "Remove from WatchList" : "Add to WatchList"}
          </button>
        </div>
      </div>
    </div>
